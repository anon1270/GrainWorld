...
def sampling_4d(sample_points, mlvl_feats, scale_weights, occ2img, fut2cur, image_h, image_w, num_views=6, eps=1e-5):
    """
    Args:
        ...
        fut2cur: 4x4 projection matrix (fut_occ2cur_occ) in shape [B, 4, 4]
    Symbol meaning:
        B: batch size
        Q: num of queries
        T: num of frames
    ...
    B, Q, T, G, P, _ = sample_points.shape  # [B, Q, T, G, P, 3], B=8*FT
    N = num_views
    ...
    occ2img = occ2img[:, :, None, None, :, :]  # [B, TN=48, 1, 1, 4, 4]
    occ2img = occ2img.expand(B, T*N, Q, G * P, 4, 4)
    occ2img = occ2img.reshape(B, T, N, Q, G*P, 4, 4)  # [8*FT, 8, 6, 600, 16, 4, 4]
    ...
    sample_points = sample_points.transpose(1, 3)   # [B, T, N, Q, GP, 4, 1], [8*FT, 8, 6, 600, 16, 4, 1]

    # project 3d sampling points to N views
    if fut2cur != None:
        # sample_points_cam = torch.matmul(fut2cur, sample_points)
        fut2cur = torch.cat(fut2cur, dim=0)
        sample_points = torch.einsum('bij,bacdejk->bacdeik', fut2cur, sample_points)
        # 'bij,bacdejk->bacdeik':
        # - b: batch 8*FT
        # - i,j: (4x4)
        # - a,c,d,e: sample_points' middle dim (8,6,600,16)
        # - j,k: last2 (4,1)
    ...
    # split
    B_original = B // FT
    sample_points_cam_splits = torch.split(sample_points_cam, B_original * T * G, dim=0)
    scale_weights_splits = torch.split(scale_weights, B_original * G * T, dim=0)

    # multi-scale multi-view grid sample
    # final = msmv_sampling(mlvl_feats, sample_points_cam, scale_weights)
    final_splits = []
    for i in range(FT):
        final_split = msmv_sampling(mlvl_feats, sample_points_cam_splits[i], scale_weights_splits[i])
        # final_split = msmv_sampling_pytorch(mlvl_feats, sample_points_cam_splits[i], scale_weights_splits[i])
        final_splits.append(final_split)

    final = torch.cat(final_splits, dim=0)
