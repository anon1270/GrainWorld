...
class SparseWorld(MVXTwoStageDetector):
    def __init__(self,
        ...
        # self.dino_img_backbone = AutoModel.from_pretrained(dino_img_backbone)
        # self.freeze(self.dino_img_backbone.layer_norm)
    ...
    def extract_img_feat(self, img):
        ...
        '''
        img_feats[0].shape: torch.Size([48, 256, 64, 176])
        img_feats[1].shape: torch.Size([48, 512, 32, 88])
        img_feats[2].shape: torch.Size([48, 1024, 16, 44])
        img_feats[3].shape: torch.Size([48, 2048, 8, 22])
        '''

        # dinov3
        # outs = self.dino_img_backbone(img, output_hidden_states=True)['hidden_states']
        # img_feats = list()
        # for i in range(1, len(outs)):
        #     img_feats.append(outs[i])
        ...
        '''
        img_feats[0].shape: torch.Size([48, 256, 64, 176])
        img_feats[1].shape: torch.Size([48, 256, 32, 88])
        img_feats[2].shape: torch.Size([48, 256, 16, 44])
        img_feats[3].shape: torch.Size([48, 256, 8, 22])
        '''
    ...
    def extract_feat(self, img, img_metas):
        ...
        if self.data_aug is not None:
            ...
            if 'img_norm_cfg' in self.data_aug:
                ...
                # img = img / 255.0  # caution: for dino version
    ...
    def forward_train(self,
        ...
        outs = self.pts_bbox_head(img_feats, img_metas, fut2cur, fut_list)

        voxel_semantics = torch.cat(voxel_semantics, dim=0)
        mask_camera = torch.cat(mask_camera, dim=0)
        # The released head has no scene-end validity policy.  Accept the
        # loader's optional fixed-shape padding metadata so legacy SparseWorld
        # configs remain callable, but leave their loss algebra unchanged.
        # Screening variants consume this field in their batch-safe override.
        _ = future_valid
    ...
    def forward_test(self, img_metas, fut2cur, fut_list, img=None, **kwargs):
        ...
        return self.simple_test(img_metas[0], fut2cur[0], fut_list[0], img[0], **kwargs)

    def simple_test_pts(self, x, img_metas, fut2cur, fut_list, rescale=False):
        outs = self.pts_bbox_head(x, img_metas, fut2cur, fut_list)
        return self.pts_bbox_head.get_occ(outs, img_metas[0], rescale=rescale)
    
    def simple_test(self, img_metas, fut2cur, fut_list, img=None, rescale=False):
        world_size = get_dist_info()[1]
        if world_size == 1:  # online
            return self.simple_test_online(img_metas, fut2cur, fut_list, img, rescale)
        else:  # offline
            return self.simple_test_offline(img_metas, fut2cur, fut_list, img, rescale)

    def simple_test_offline(self, img_metas, fut2cur, fut_list, img=None, rescale=False):
    ...
    def simple_test_online(self, img_metas, fut2cur, fut_list, img=None, rescale=False):
        ...
        per_view_meta_keys = {
            k for k, value in img_metas[0].items()
            if isinstance(value, list) and len(value) == 6 * num_frames
            and not k.startswith('trajectory_eval_')
            and k not in ('future_valid', 'fut_list', 'fut2cur')
        }
        ...
        for i in range(num_frames):
            ...
            for k, item in img_metas[0].items():
                if k in per_view_meta_keys:
                    img_metas_curr[0][k] = [item[j] for j in img_indices]
                elif isinstance(item, list):
                    img_metas_curr[0][k] = list(item)
                else:
        ...
        img_metas_reorganized = [dict(img_metas_list[0][0])]
        for k in per_view_meta_keys:
            img_metas_reorganized[0][k] = [
                value for frame_metas in img_metas_list
                for value in frame_metas[0][k]
            ]
